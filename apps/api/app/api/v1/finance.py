...
async def _vehicle(session: DatabaseSession, organization_id: UUID, vehicle_id: UUID, *, lock: bool = False) -> Vehicle:
    statement = select(Vehicle).where(Vehicle.id == vehicle_id, Vehicle.organization_id == organization_id)
    if lock:
...
async def _validate_trip(session: DatabaseSession, organization_id: UUID, trip_id: UUID | None, vehicle_id: UUID | None) -> None:
    if trip_id is None:
        return
...
async def _fuel_read(session: DatabaseSession, log: FuelLog) -> FuelRead:
    ...
    return FuelRead(
        fuel_id=log.id, vehicle_id=vehicle.id, vehicle_reg_no=vehicle.reg_no,
        trip_id=log.trip_id, fuel_date=log.fuel_date, liters=log.liters,
        cost=log.cost, odometer=log.odometer, version=log.version,
    )


@router.get("/fuel", response_model=list[FuelRead])
async def list_fuel(principal: CurrentPrincipal, session: DatabaseSession, vehicle_id: Annotated[UUID | None, Query()] = None) -> list[FuelRead]:
...
async def create_fuel(payload: FuelCreate, principal: CurrentPrincipal, session: DatabaseSession) -> FuelRead:
    ...
    if payload.odometer is not None and payload.odometer < vehicle.odometer:
        raise AppError(code="ODOMETER_REGRESSION", message="Odometer cannot decrease.", status_code=409)
    log = FuelLog(organization_id=principal.organization_id, **payload.model_dump(exclude={"fuel_date"}), fuel_date=payload.fuel_date or date.today())
    session.add(log)
    await session.flush()
    settings = await session.get(OrganizationSettings, principal.organization_id)
    session.add(Expense(organization_id=principal.organization_id, vehicle_id=vehicle.id, trip_id=payload.trip_id, fuel_id=log.id, category="FUEL", amount=payload.cost, currency=settings.currency if settings else "INR", status="APPROVED", approved_by=principal.user_id, approved_at=datetime.now(UTC)))
...
async def list_expenses(principal: CurrentPrincipal, session: DatabaseSession, vehicle_id: Annotated[UUID | None, Query()] = None) -> list[Expense]:
...
async def create_expense(payload: ExpenseCreate, principal: CurrentPrincipal, session: DatabaseSession) -> Expense:
    ...
    settings = await session.get(OrganizationSettings, principal.organization_id)
    expense = Expense(organization_id=principal.organization_id, currency=settings.currency if settings else "INR", **payload.model_dump())
    session.add(expense)
    await session.commit()
...
async def _decide(expense_id: UUID, payload: ExpenseDecision, principal: CurrentPrincipal, session: DatabaseSession, target: str) -> Expense:
    expense = (
        await session.execute(
            select(Expense).where(Expense.id == expense_id, Expense.organization_id == principal.organization_id).with_for_update()
        )
    ).scalar_one_or_none()
    if expense is None:
    ...
    if expense.status != "PENDING" or expense.version != payload.version:
        raise AppError(code="EXPENSE_CONFLICT", message="Expense cannot be decided.", status_code=409)
    ...
    session.add(AuditLog(organization_id=principal.organization_id, actor_id=principal.user_id, action=target.lower(), entity_type="expense", entity_id=expense.id))
    await session.commit()
    await session.refresh(expense)
    return expense
...
async def approve_expense(expense_id: UUID, payload: ExpenseDecision, principal: CurrentPrincipal, session: DatabaseSession) -> Expense:
...
async def reject_expense(expense_id: UUID, payload: ExpenseDecision, principal: CurrentPrincipal, session: DatabaseSession) -> Expense:
