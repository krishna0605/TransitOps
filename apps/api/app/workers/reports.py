    ...
    async with session_factory() as session:
        ...
        try:
            ...
            for expense in expenses:
                writer.writerow([expense.id, expense.category, expense.amount, expense.currency, expense.created_at.isoformat()])
            ...
            session.add(Notification(organization_id=report.organization_id, user_id=report.requested_by, type="REPORT_READY", title="Report ready", message="Your TransitOps export is ready.", payload={"report_id": str(report.id)}))
            await session.commit()
